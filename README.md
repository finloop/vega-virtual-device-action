...
| [`.github/workflows/`](.github/workflows/) | `docker-publish.yml` (build + push both images to GHCR), `vvd-action-test.yml` (end-to-end smoke test of the action) and `vvd-navigation-test.yml` (navigate the Kepler Video App and screenshot each step). |
